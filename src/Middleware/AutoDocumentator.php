...
/**
 * Please note that TornadoRouterAutoDocumentation is instantiated twice
 * once as a BaseMiddleware, another time: as callback
 */
/**
 * Clase que permite autodocumentar el codigo de manera automatica obteniendo datos de los comentarios
 */
class AutoDocumentator extends \TornadoRouter\BaseMiddleware {
  private $path = '/docs';
  ...
  function preprocess(&$router) {
    $router->addRoute(array(
      'path' => $this->path,
      'get'  => array('\TornadoRouter\Middleware\AutoDocumentator', 'generateDocs'),
    ));
  }
