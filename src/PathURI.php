<?php
	...
	class PathURI {
    	...
    	public function getRegularExpression() {
        	$expression = $this->pathuri;
        	//si la ruta tiene definido algun parametro dinamico en su deficion, como ejemplo: /path/{hola}
        	//entonces modificara la expresión regular para como la siguiente: ~^/path/(?P(?:/?[^/]*))$~
        	if (preg_match_all('~(?P<match>\{(?P<name>.+?)\})~', $expression, $matches)) {
            	$expressions = array_map(array($this, 'getPatternExpression'), $matches['name']);
    	 ...
    	 */
    	public function getPatternExpression($token, $pattern = null) {
        	if ($pattern) {
	        ...
	        foreach($matches as $k => $v) {
	            ...
	            if (is_numeric($k)) {
	                ...
	                if (isset($this->callbacks[$k])) {
	                    $callback = CallbackUtil::getCallback($this->callbacks[$k]);
